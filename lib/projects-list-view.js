...
export default class ProjectsListView extends SelectListView {
  ...
  show(projects) {
    ...
    for (project of projects) {
      ...
      try {
        let stats = fs.statSync(project.props.paths[0]);
  ...
  viewForItem({_id, title, group, icon, devMode, paths, pathMissing}) {
    let showPath = this.showPath;

    if (pathMissing) {
      icon = 'icon-alert';
      group = 'Path not available';
    }

    return $$(function() {
      this.li({class: 'two-lines'},
      {'data-project-id': _id, 'data-path-missing': pathMissing}, () => {
