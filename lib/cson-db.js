...
export default class CSONDB {
  ...
  find(callback=null) {
    this.readFile(results => {
      ...
      for (key in results) {
        ...
        const localSettings = this.readLocalSettings(result.paths[0]);
        if (localSettings) {
          result = _.deepExtend(result, localSettings);
        }
  ...
  readFile(callback) {
    fs.existsSync(this.file(), (exists) => {
      if (exists) {
        try {
        ...
        fs.writeFileSync(this.file(), '{}', () => callback({}));
      }
    });
