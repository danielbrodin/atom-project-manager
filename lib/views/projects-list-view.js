...
export default class ProjectsListView extends SelectListView {
  ...
  viewForItem(project) {
    const { title, group, icon, devMode, paths, source } = project.props;
    ...
    return $$(function itemView() {
      ...
      { 'data-path-missing': projectMissing }, () => {
        this.div({ class: 'primary-line' }, () => {
          ...
          if (source === 'git') {
            const { ahead, behind } = project.aheadBehindCount;
            this.span({ class: 'project-manager-repository' }, () => {
              this.span({ class: 'icon icon-repo-push' }, () => {
                this.span(`${ahead}`);
              });
              this.span({ class: 'icon icon-repo-pull' }, () => {
                this.span(`${behind}`);
              });
            });
          }
