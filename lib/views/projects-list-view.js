...
export default class ProjectsListView extends SelectListView {
  ...
  initialize() {
    ...
    infoElement.innerHTML = 'shift+enter will open project in the current window';
  ...
  sortItems(items) {
    ...
    if (key === 'default') {
      ...
      items.sort((a, b) => {
        const aModified = a.project.lastModified.getTime();
        const bModified = b.project.lastModified.getTime();
